...
p_cut <- 0.01
min_gene_set <- 5
coef <- "stimulationcsp:malaria_status1"
form_dis <- "~plate + total_reads + age_weeks + stimulation*malaria_status"
...
**disease filtering   :** rtss, both age groups, dmso and hbs 
...
countSampleSizes(pData(mal_dis), "visit", 
    c("stimulation", "vaccine", "malaria_status", "age")) %>%
headerKable(labels=c("stimulation", "vaccine", "malaria_status", "age"), levels=c(2,2,2,2), 
...
cam_m0 <- as.tibble(camera(mal_m0, btm_ind, design=des_m0, contrast=coef),
                       rownames="geneset")
...
cam_m3 <- as.tibble(camera(mal_m3, btm_ind, design=des_m3, contrast=coef),
                       rownames="geneset")
